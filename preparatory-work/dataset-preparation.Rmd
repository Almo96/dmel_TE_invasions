...
## deviaTE_analysis
We used the deviaTE_analysis in parallel, this generated a file for each TE in the reference for every sample, we have 180 TEs, 3 single-copy genes for the normalization and 122 samples.
```{bash, eval=FALSE}
...
## Plotting
Finally we used deviTE_plot for generating the plots of the TEs of interest:
```{bash, eval=FALSE}
for file in *.OPUS; do deviaTE_plot --input "$file" & done
```

We also generated all the TEs for Harwich to screen for the reads distribution and remove outliers
```{bash, eval=FALSE}
